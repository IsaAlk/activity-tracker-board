...
*/
  ...
  sprintf(message, "GYRO-x:%.2f-y:%.2f-z:%.2f$ACCELERO-x:%d-y:%d-z:%d",
          pGyroDataXYZ[0], pGyroDataXYZ[1], pGyroDataXYZ[2], pDataXYZ[0],
...
private:
  ...
  virtual void onDataWritten(const GattWriteCallbackParams &params) {
    if ((params.handle == _writable_characteristic->getValueHandle()) &&
      ...
      printf("New characteristic value wPritten: %x\r\n", *(params.data));
      ...
      if (ntp_client(net) < 0) {
        printf("Connection error < 0: %d", ret);
      }
      ...
      state = iotc_connect(
          iotc_context, /*username=*/NULL, /*password=*/jwt,
      ...
      /* Regularly call the function iotc_events_process_blocking() to process
         connection requests, and for the client to regularly check the sockets
         for incoming data in endless loop.
      */
